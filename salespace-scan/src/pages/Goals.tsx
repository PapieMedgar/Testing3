...
const Goals = () => {
  ...
  const { data: goals = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const goals = user.role === 'AGENT' 
        ? await goalAPI.getPersonalGoals(user.id)
        : await goalAPI.getGoals({ active: true });
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            {user?.role === 'AGENT' 
              ? 'View your assigned goals and progress'
        ...
        <div className="text-center py-8">
          ...
          <p className="text-muted-foreground mb-4">
            {user?.role === 'ADMIN' 
              ...
              : user?.role === 'AGENT'
                ? "You don't have any goals assigned to you yet"
                        ...
                        <TableRow key={goal.id}>
                          ...
                          <TableCell>
                            <div className="space-y-2">
                              ...
                              <Progress value={getGoalProgress(goal.progress)} />
                            </div>
                          </TableCell>
                          ...
                          <TableCell>
                            <div className="text-sm">
                              {goal.region ? `Region: ${goal.region}` : ''}
                              {goal.shop_id ? `Shop: ${goal.shop_id}` : ''}
                              {!goal.region && !goal.shop_id && 'All'}
                            </div>
                          </TableCell>
                          {user?.role === 'ADMIN' && (
                            <TableCell>
