      ...
      <ScrollArea className="h-[600px] w-full p-4">
        <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {visit.brand && (
                  <div className="flex flex-col p-2 bg-white rounded-md shadow-sm">
                    ...
                    <span className="font-semibold text-blue-700">{visit.brand}</span>
                  </div>
                )}
                {visit.category && (
                  <div className="flex flex-col p-2 bg-white rounded-md shadow-sm">
                    <span className="text-sm text-muted-foreground font-medium">Category</span>
                    <span className="font-semibold text-blue-700">{visit.category}</span>
                  </div>
                )}
                {visit.product && (
                  <div className="flex flex-col p-2 bg-white rounded-md shadow-sm">
                    <span className="text-sm text-muted-foreground font-medium">Product</span>
                    <span className="font-semibold text-blue-700">{visit.product}</span>
                  </div>
                )}
              </div>
            </Card>
          ...
          <div className="space-y-4">
            <Card className="p-4">
              <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                <ClipboardList className="h-5 w-5 text-indigo-600" /> Visit Response Data
              </h3>
              
              {visit.surveyResponses.length === 0 ? (
                <div className="text-center py-4 text-muted-foreground">
                  No response data available for this visit
                </div>
              ) : (
                <div className="space-y-6">
                  {visit.surveyResponses.map((response, index) => (
                    <div key={index} className="border-b pb-4 last:border-b-0 last:pb-0">
                      <h4 className="text-md font-semibold mb-3 capitalize bg-blue-50 p-2 rounded">
                      ...
                      {typeof response.answer === "object" && response.answer !== null ? (
                        <div className="space-y-4">
                          {Object.entries(response.answer).map(([key, value]) => (
                            <div key={key}>
                              <h5 className="text-sm font-medium mb-2 capitalize bg-gray-50 p-1 pl-2 rounded">
                                {key.replace(/([A-Z])/g, ' $1').toLowerCase()}
                              </h5>
                              {typeof value === "object" && value !== null ? (
                                <div className="space-y-2 ml-4">
                                  {Object.entries(value).map(([subKey, subValue]) => (
                                    <div key={subKey} className="grid grid-cols-2 gap-4 border-b border-gray-100 pb-2 last:border-b-0 last:pb-0">
                                      ...
                                      <div className="text-sm">
                                        {typeof subValue === 'object' && subValue !== null ? (
                                          <div className="ml-4 text-sm">
                                            <pre className="whitespace-pre-wrap bg-gray-50 p-2 rounded text-xs">{JSON.stringify(subValue, null, 2)}</pre>
                                          </div>
                                        ) : (
                                ...
                                <div className="ml-4 text-sm grid grid-cols-2 gap-4">
                                  <div className="text-sm text-muted-foreground capitalize font-medium">Value:</div>
                                  <div>
                        ...
                        <div className="ml-4 text-sm grid grid-cols-2 gap-4">
                          <div className="text-sm text-muted-foreground capitalize font-medium">Response:</div>
                          <div>
